...
public:
	...
	{
    //topic sub:
		image_sub_depth = it_.subscribe( "/simulator/camera_node/image/depth",
						 1, &ImageConverter::imageDepthCb, this );
		image_sub_color = it_.subscribe( "/simulator/camera_node/image/compressed", 1,
	...
	void imageColorCb( const sensor_msgs::ImageConstPtr &msg )
	{
		...
		float	real_z	= 0.001 * depthImage.at<u_int16_t>( 0, 0 );
		float	real_x	=
			(0 - camera_info.K.at( 2 ) ) / camera_info.K.at( 0 ) * real_z;
		float real_y =
			(0- camera_info.K.at( 5 ) ) / camera_info.K.at( 4 ) * real_z;
